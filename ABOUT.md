...
Many students learn in classrooms where internet is unreliable, cloud AI is unaffordable, and power cuts are normal. Generic chatbots also miss what teachers actually need: **scenario-based practice**, **explanations at the right level**, and answers grounded in **local curriculum materials**.

We built **Arapai** for the [Africa Deep Tech Challenge 2026 — Laptop LLM Challenge](https://africadeeptech.org/challenge-2026): an on-device tutor that runs on an ordinary 8 GB laptop with no cloud dependency. The name reflects a simple goal — **AI that schools can own, run, and trust offline**.
...
| `cbc_engine.py` | Hybrid keyword + LLM rubric scoring |
